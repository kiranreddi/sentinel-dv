...
**Current release: v1.3.2** ([changelog](../about/changelog.md))
...
> **Note:** Do not install PyPI **1.0.0** (broken wheel). Use **`sentinel-dv>=1.3.2`** for VCS/Questa/Cadence fixtures, assertion/coverage intelligence, VCD/JSON waveforms, and all 15 MCP tools. Verified flow: install → `sentinel-dv-index --index-all` → `sentinel-dv-server` → MCP tools.
